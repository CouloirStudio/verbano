...
class OpenAIService {
     ...
     */
    public async generateReport(tts: string, context: string): Promise<string | null> {
        try {
    ...
    public async generateTags(tts: string): Promise<string[] | undefined> {
        try {
            const completion = await this.openai.chat.completions.create({
                model: 'gpt-3.5-turbo',
                messages: [
                    { role: "system", content: "You will generate a list of 4 content tags related to the following text split by commas:" },
                    { role: "user", content: tts },
                ],
            });
    ...
    public async generateTitle(tts: string): Promise<string | null> {
        try {
            const completion = await this.openai.chat.completions.create({
                model: 'gpt-3.5-turbo',
                messages: [
                    { role: "system", content: "You will generate a title for the following text:" },
                    { role: "user", content: tts },
                ],
            });
            return completion.choices[0].message.content;
